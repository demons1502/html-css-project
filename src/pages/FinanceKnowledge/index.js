...
import { getArticlesData, mostViewArticles } from '../../slices/financeKnowledge';
        ...
        <Spin spinning={loading === LOADING_STATUS.pending}>
          <Row gutter={[10, 10]}>
            <Col lg={8} md={24} sm={24}>
              <Row>{mostViewData[0] && <FinanceKnowledgeCard content={mostViewData[0]} wrap lg={24} />}</Row>
            </Col>
            <Col lg={16} md={24} sm={24}>
              <Row gutter={[10, 10]} align="stretch" style={{ height: '100%' }}>
                {mostViewData.slice(1)?.map((item) => (
                  <FinanceKnowledgeCard content={item} key={item.id} lg={12} md={12} />
                ))}
              </Row>
            </Col>
          </Row>
      ...
      <div>
        ...
        <Spin spinning={loading === LOADING_STATUS.pending}>
          <List
            ...
            renderItem={(item) => (
              <List.Item>
                <FinanceKnowledgeCard content={item} key={item.id} lg={24} showImage={false} />
              </List.Item>
            )}
          ></List>
          <Pagination total={articlesData.articlesCount} setPaginate={setPaginate} showSizeChanger={false}></Pagination>
        </Spin>
      </div>
    </div>
  );
