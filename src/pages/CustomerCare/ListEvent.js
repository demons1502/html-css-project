...
export default function ListEvent() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record) => {
        return (
          <span className="cursor-pointer" onClick={() => editModal(record)}>
            {record.name}
          </span>
        );
      },
      ...
      render: (record) => {
        return (
          <div className="btn-table">
            <Button
              className="btn-table__btn btn-table-sms m-r-10"
              icon={<img src={IconSms} alt="" />}
              onClick={() => showModalSms(record.id)}
            >
              {t('customer care.sms')}
            </Button>
            <Button
              className="btn-table__btn btn-table-email m-r-10"
              icon={<img src={IconMessage} alt="" />}
              onClick={() => showModalEmail(record.id)}
            >
              {t('customer care.email')}
            </Button>
            <Popconfirm
              className="pop-confirm-delete"
              placement="top"
  ...
  useEffect(() => {
    if (!customerData?.isPotential) {
      ...
      const index = percents.findIndex((item) => item.value === customerData?.status);
      setPercent(percents[index]?.percent);
    }
  }, [customerData]);
  ...
  return (
    <>
      ...
      {customerData.customerId > 0 && (
        <div className="customer-care__center--footer">
          <S.ButtonAdd icon={<img src={IconPlus} alt="" />} onClick={() => addModal(true)}>
            {t('customer care.add event template')}
          </S.ButtonAdd>
          <S.ButtonAdd icon={<img src={IconPlus} alt="" />} onClick={() => addModal(false)}>
            {t('customer care.add event')}
          </S.ButtonAdd>
        </div>
      )}
      <Modal
        isVisible={visibleModalAddEvent}
        ...
        content={
          <AddEventContent
            ...
            setVisibleModalAddEvent={setVisibleModalAddEvent}
          />
        }
      />
      <Modal
        isVisible={visibleModalEmail}
        setIsVisible={setVisibleModalEmail}
        title={t('customer care.email title')}
        width={770}
        content={<SendEmailContent eventId={eventId} setVisibleModalEmail={setVisibleModalEmail} />}
      />
      <Modal
        isVisible={visibleModalSms}
        setIsVisible={setVisibleModalSms}
        title={t('customer care.sms title')}
        width={770}
        content={<SendSmsContent eventId={eventId} setVisibleModalSms={setVisibleModalSms} />}
      />
    </>
  );
}
