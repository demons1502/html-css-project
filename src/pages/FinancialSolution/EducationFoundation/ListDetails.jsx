...
const ListDetails = () => {
  ...
  var { getSpeechScript } = useSelector((state) => state.financialSolution)
  const [page, setPage] = useState(1);
  const [limit, setLimit] = useState(1);
  if (getSpeechScript) {
    var { objective, procedure, dialogues } = getSpeechScript;
  }
  useEffect(() => {
      ...
      if (page === 1) {
        return (
          <p style={{ fontSize: '14px', display: 'flex', alignItems: 'baseline' }}>
            {" "}
            <img src={DotImg} alt="dot" /><span dangerouslySetInnerHTML={{ __html: objective }} />
          </p>
        )
      } else if (procedure) {
        if (page === 2) {
          return (
            <p style={{ fontSize: '14px', display: 'flex', alignItems: 'baseline' }}>
              {" "}
              <img src={DotImg} alt="dot" /><span dangerouslySetInnerHTML={{ __html: procedure }} />
            </p>
          )
        } else {
  ...
  return (
    <Card className="content-div-2-header" title={t("survey.rttitle")}>
      <div className="content-div-2-content" style={{paddingBottom: '20px'}}>
...
const DialogItem = ({ dialogues }) => {
  return dialogues?.map((dialog, i) => (
    <div key={i}>
      <img src={DotImg} alt="dot" /> <span dangerouslySetInnerHTML={{ __html: dialog?.text }}></span>
    </div>
  ));
};